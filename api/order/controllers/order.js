...
/**
 *returns a single order that belongs to the logged in user
 * @param {any} ctx
 */
  async findOne(ctx) {
    const {id} = ctx.params
  ...
  /**
   * Creates an order and sets up Stripe checkout session for frontend
   * @param {any} ctx
   */
  async create(ctx){
      ...
      if(!product){
        return ctx.throw(400, 'Please specify product')
      }
  ...
  /**
   *  Given a checkout session, verify the payment with Stripe and update the order
   * @param {any} ctx
   */
  async confirm(ctx) {
    ...
    if(session.payment_status === 'paid'){
      ...
      ctx.throw(400, "Payment was not successful, please call support.")
    }
  }
};
